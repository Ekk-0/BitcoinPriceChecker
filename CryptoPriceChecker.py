...
class PriceChecker():
    ...
    def displayList(self):
        ...
        self.levelsList = sorted(self.levelsList, reverse = True)
        # Print the items in the list (Based on the above sort, numbers should appear from large to small.)
        for index in range(len(self.levelsList)):
            print("$" + '{:,}'.format((self.levelsList[index])))
    ...
    def displayMenu(self):
        ...
        if(self.currentPrice > 0):
            ...
            print("4. Display the current Bitcion price here")
    ...
    def writeLevelsToFile(self):
        ...
        with open("levels.txt", "w") as self.levelsFile:
            ...
            for item in self.levelsList:
                ...
                self.levelsFile.write(str(item) + "\n")
        # Close the file
        self.levelsFile.close()
    ...
    def monitorLevels(self):
        ...
        #           ['Previou Prsice: 9688.69',    9688.69], 
        ...
        # Append the sub-List to displayList.
        previousPriceLabel = f'{bcolors.OKBLUE}Previous Price: {bcolors.ENDC}' + f'{bcolors.OKBLUE}' + (str(self.previousPrice)) + f'{bcolors.ENDC}'
        OldList = [previousPriceLabel, self.previousPrice]
        displayList.append(OldList)
        ...
        if self.currentPrice > self.previousPrice:
            currentPriceLabel = f'{bcolors.OKGREEN}Current Price: {bcolors.ENDC}' + f'{bcolors.OKGREEN}' + str(self.currentPrice) + f'{bcolors.ENDC}'
        if self.currentPrice < self.previousPrice:
            currentPriceLabel = f'{bcolors.OKRED}Current Price: {bcolors.ENDC}' + f'{bcolors.OKRED}' + str(self.currentPrice) + f'{bcolors.ENDC}'
        if self.currentPrice == self.previousPrice:
            currentPriceLabel = f'{bcolors.OKBLUE}Current Price: {bcolors.ENDC}' + f'{bcolors.OKBLUE}' + str(self.currentPrice) + f'{bcolors.ENDC}'
        CurrentList = [currentPriceLabel, self.currentPrice]
        displayList.append(CurrentList)
        ...
        for i in range(0,len(displayList)):
            ...
            if 'Price Level' in displayList[i][0]:
                ...
                if(
                    (priceLevel <= self.previousPrice and priceLevel >= self.currentPrice) or (priceLevel >= self.previousPrice and priceLevel <= self.currentPrice)
                ):
                    ...
                    print(f'{bcolors.OKGREEN}Alarm!{bcolors.ENDC}')
...
while userInput != 0:
    ...
    if(userInput == 1):
        ...
        userInput = 0 # prevent the app from continuing if the user pressed Ctrl+C to stop item
